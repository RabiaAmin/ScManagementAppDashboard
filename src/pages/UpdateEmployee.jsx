...
export default function UpdateEmployee() {
  ...
  // ================= LOAD EMPLOYEE =================
  useEffect(() => {
    dispatch(getSingleEmployeeDetails(id));
  }, [dispatch, id]);
  ...
  useEffect(() => {
    if (employee) {
      ...
      setDepartment(employee.department || "OTHER");
      setPayType(employee.payType || "TIME_BASED");
      setEmploymentType(employee.employmentType || "PERMANENT");
      setWeeklyPayment(employee.weeklyPayment || "");
      setHourlyRate(employee.hourlyRate || "");
      setDailyRate(employee.dailyRate || "");
      setPerPieceRate(employee.perPieceRate || "");
      setMonthlySalary(employee.monthlySalary || "");
      setPaymentMethod(employee.paymentMethod || "CASH");
    }
  }, [employee]);
  ...
  const updateEmployeeHandler = (e) => {
    ...
    if (payType === "TIME_BASED") {
      formData.append("weeklyPayment", weeklyPayment);
    ...
    <div className="w-full h-[100vh] p-4 sm:p-6 md:p-8">
      <div className="grid gap-6 max-w-5xl mx-auto">
        <div className="grid gap-2">
          ...
          <p className="text-muted-foreground">Edit Employee Details</p>
        </div>

        <form onSubmit={updateEmployeeHandler} className="grid gap-6">
            ...
            {payType === "TIME_BASED" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input type="number" value={weeklyPayment} onChange={(e) => setWeeklyPayment(e.target.value)} />
                </div>
